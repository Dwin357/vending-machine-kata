...
RSpec.describe Coin do
  ...
  describe 'initialize' do
    ...
    it 'sets a weight attribute from legal tender' do
      expect(subject.weight).to eq LEGAL_TENDER
        .fetch(:nickel).fetch(:properties).fetch(:weight)
    end
    it 'sets a diameter attribute from legal tender' do
      expect(subject.diameter).to eq LEGAL_TENDER
        .fetch(:nickel).fetch(:properties).fetch(:size)
    end
